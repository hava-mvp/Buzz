...
messageBird.checkArrLength = function(arr, barNameClean, offerClean, endTimeClean, callback) {
  ...
  var params = {
    'originator': 'Hava',
    'body': `Hello! ${barNameClean} has a new offer!!
    Offer: ${offerClean}.
  ...
  for (var i=0; i<arr.length; i+=50) {
    ...
    messageBird.sendOfferAlert(batchesOf50, params, messageSentStatus, callback);
  }
}

messageBird.sendOfferAlert = function(batchOf50, params, messageSentStatus, callback) {
  params['recipients'] = batchOf50,

  messagebird.messages.create(params, function (err, response) {
    if (err) {
      // console.log('MESSAGE BIRD ERR>>>>>>',err);
      messageBird.haveAllTextsBeenSent('notOk', messageSentStatus, callback);
    } else {
      // console.log('MESSAGE BIRD RESPONSE>>>>>>>',response);
      messageBird.haveAllTextsBeenSent('ok', messageSentStatus, callback);
    }
  });
}

messageBird.haveAllTextsBeenSent = function(response, messageSentStatus, callback){
