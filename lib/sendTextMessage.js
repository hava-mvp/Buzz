  ...
  getAllRecipients(function(recipients) {
    ...
    function sendOfferAlert (arr, remaining) {
      var params = {
        'originator': 'Hava',
        'recipients': [arr],
        'body': 'Hello! ' + barNameClean + ' has a new offer: ' + offerClean + '. The offer ends at ' + endTimeClean + '. Click the link below to get your code. http://hava-mvp.herokuapp.com/public/#live-offers'
      };
...
function getAllRecipients(callback) {
  ...
  firebaseRef.once('value', function(snapshot) {
    ...
    function getPhoneNumber(elem, index, arr) {
      count++;
      firebaseRef.child(elem + "/phoneNumber").once("value", function(snapshot) {
